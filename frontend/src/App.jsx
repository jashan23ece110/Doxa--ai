...
function App() {
  ...
  const ResultPanel = ({ title, data, delay }) => (
    <motion.div
      ...
      className="bg-[#0d1220] border panel-glow rounded-xl flex flex-col overflow-hidden shadow-sm h-full"
    >
      <div className="flex items-center justify-between px-4 py-3 border-b border-[rgba(0,217,255,0.1)] bg-[#0a0e17]">
        <h3 className="font-medium text-[14px] text-white" style={{ fontFamily: 'Rajdhani, sans-serif' }}>{title}</h3>
        <div className="flex items-center gap-2">
          {data?.latency_ms && (
            <span className="text-[11px] px-2 py-0.5 bg-[#0a0e17] border panel-glow rounded text-[#5a6d82]" style={{ fontFamily: 'JetBrains Mono, monospace' }}>{data.latency_ms}ms</span>
          )}
          {data && (
            <span className={`text-[11px] font-medium px-2 py-0.5 rounded border ${data._useRag ? 'bg-[rgba(0,217,255,0.1)] border-[rgba(0,217,255,0.2)] text-[#00d9ff]' : 'bg-[#0a0e17] border-[rgba(0,217,255,0.1)] text-[#5a6d82]'}`}>
      ...
      <div className="flex-1 p-4 overflow-y-auto hud-scrollbar">
        {loading ? <SkeletonLoader /> : data ? (
          data.error
            ...
            : <div className="text-[14px] text-[#c8d6e5] leading-relaxed whitespace-pre-wrap" style={{ fontFamily: 'Rajdhani, sans-serif' }}>{data.content}</div>
        ) : (
          <div className="h-full flex items-center justify-center text-[#5a6d82] italic text-sm">Awaiting evaluation…</div>
        )}
      </div>
      ...
      <AnimatePresence>
        {retrievedContext?.length > 0 && (
          <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} className="border-t border-[rgba(0,217,255,0.1)] overflow-hidden">
            <button onClick={() => setShowContext(!showContext)} className="w-full flex items-center justify-between px-4 py-2 text-[12px] text-[#5a6d82] hover:bg-[#0d1220] transition-colors">
              ...
              {showContext && (
                <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} className="flex flex-col gap-2 px-4 pb-3 max-h-44 overflow-y-auto hud-scrollbar">
                  {retrievedContext.map((c, i) => (
                    <div key={i} className="p-2.5 bg-[#0a0e17] rounded border panel-glow text-[12px]">
                      <div className="flex justify-between mb-1">
                        <span className="text-[#00d9ff] font-medium flex items-center gap-1"><FileText className="w-3 h-3" />{c.filename || 'Unknown'}</span>
                        <span className="text-[#5a6d82]" style={{ fontFamily: 'JetBrains Mono, monospace' }}>{c.similarity ? `${(c.similarity * 100).toFixed(1)}%` : ''}</span>
                      </div>
                      <p className="text-[#c8d6e5] leading-relaxed whitespace-pre-wrap">{c.text}</p>
                    </div>
                  ))}
                </motion.div>
              )}
  ...
  if (!user) {
    return (
      <div className="min-h-screen bg-[#0a0e17] flex flex-col justify-center py-12 px-4 sm:px-6 lg:px-8 selection:bg-[rgba(0,217,255,0.2)]" style={{ fontFamily: 'Rajdhani, sans-serif' }}>
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.2 }} className="sm:mx-auto sm:w-full sm:max-w-md">
          <div className="flex justify-center mb-6">
            <div className="p-3 bg-[#0d1220] rounded-xl border panel-glow"><Sparkles className="w-8 h-8 text-[#00d9ff]" /></div>
          </div>
          <h2 className="text-center text-2xl font-bold text-white" style={{ fontFamily: 'Orbitron, sans-serif' }}>DOXA</h2>
          <p className="mt-2 text-center text-sm text-[#5a6d82]">
          ...
          <div className="hud-panel py-8 px-4 sm:rounded-xl sm:px-10">
            <form className="space-y-6" onSubmit={handleAuth}>
              {(authMode === 'login' || authMode === 'signup') && (
                <div>
                  <label htmlFor="email" className="block text-sm font-medium text-[#c8d6e5]">Email address</label>
                  <input id="email" name="email" type="email" required className="mt-1 block w-full px-3 py-2.5 border panel-glow rounded-lg bg-[#0d1220] text-white placeholder-[#5a6d82] input-glow sm:text-sm" style={{ fontFamily: 'JetBrains Mono, monospace' }} />
                </div>
              )}
              {authMode === 'phone' && (
                <div>
                  <label htmlFor="phone" className="block text-sm font-medium text-[#c8d6e5]">Phone number</label>
                  <input id="phone" name="phone" type="tel" required className="mt-1 block w-full px-3 py-2.5 border panel-glow rounded-lg bg-[#0d1220] text-white placeholder-[#5a6d82] input-glow sm:text-sm" style={{ fontFamily: 'JetBrains Mono, monospace' }} />
                </div>
              )}
              {authMode === 'otp' && (
                <div>
                  <label htmlFor="otp" className="block text-sm font-medium text-[#c8d6e5]">Verification Code</label>
                  <input id="otp" name="otp" type="text" required placeholder="123456" className="mt-1 block w-full px-3 py-2.5 border panel-glow rounded-lg bg-[#0d1220] text-white placeholder-[#5a6d82] input-glow sm:text-sm text-center tracking-widest text-lg" style={{ fontFamily: 'JetBrains Mono, monospace' }} />
                </div>
              )}
              <motion.button type="submit" whileHover={hoverScale} whileTap={tapScale} className="w-full py-2.5 px-4 rounded-lg text-sm font-bold text-[#0a0e17] bg-[#00d9ff] hover:bg-[#00f0ff] hover:shadow-[0_0_25px_rgba(0,217,255,0.4)] transition-all duration-100" style={{ fontFamily: 'Orbitron, sans-serif', letterSpacing: '0.1em' }}>
                {authMode === 'login' ? 'SIGN IN' : authMode === 'signup' ? 'CREATE ACCOUNT' : authMode === 'phone' ? 'SEND CODE' : 'VERIFY'}
              </motion.button>
            </form>

            <div className="mt-6">
              <div className="relative"><div className="absolute inset-0 flex items-center"><div className="w-full border-t border-[rgba(0,217,255,0.1)]" /></div><div className="relative flex justify-center text-sm"><span className="px-2 bg-[#0a0e17] text-[#5a6d82]">Or continue with</span></div></div>
              <div className="mt-6 grid grid-cols-2 gap-3">
                <button type="button" onClick={() => setAuthMode('phone')} className="w-full py-2 px-4 border panel-glow rounded-lg bg-[#0d1220] text-sm font-medium text-[#c8d6e5] hover:bg-[#141c2e] transition-colors">Phone</button>
                <button type="button" onClick={() => setAuthMode(authMode === 'login' ? 'signup' : 'login')} className="w-full py-2 px-4 border panel-glow rounded-lg bg-[#0d1220] text-sm font-medium text-[#c8d6e5] hover:bg-[#141c2e] transition-colors">
  ...
  /* ═══════════════════════════════════════════
     ...
     ═══════════════════════════════════════════ */
  return (
    <div className="h-screen w-screen bg-[#0a0e17] overflow-hidden relative" style={{ fontFamily: 'Rajdhani, sans-serif' }}>
      ...
      <AnimatePresence>
        {activeOverlay && (
          ...
          >
            ...
            >
              {/* overlay header */}
              <div className="flex items-center justify-between px-5 py-3 border-b border-[rgba(0,217,255,0.15)]">
                <h2 className="text-sm font-bold text-[#00d9ff] uppercase tracking-[0.15em]" style={{ fontFamily: 'Orbitron, sans-serif' }}>
                ...
                <button onClick={closeOverlay} className="p-1.5 text-[#5a6d82] hover:text-white transition-colors">
                  <X className="w-5 h-5" />
                </button>
              </div>
                    ...
                    <motion.section variants={staggerItem} className="hud-panel p-4 sm:p-5">
                      <form onSubmit={handleSubmit} className="flex flex-col gap-4">
                        <div className="flex items-center justify-between flex-wrap gap-2">
                          <label htmlFor="prompt" className="text-[11px] font-semibold text-[#5a6d82] tracking-widest uppercase" style={{ fontFamily: 'Orbitron, sans-serif' }}>Prompt</label>
                          <button
                            ...
                            className={`flex items-center gap-2 px-2.5 py-1.5 rounded-md text-xs font-medium transition-all border ${
                              useRag ? 'bg-[rgba(0,217,255,0.1)] text-[#00d9ff] border-[rgba(0,217,255,0.2)]' : 'bg-[#0d1220] text-[#5a6d82] border-[rgba(0,217,255,0.1)] hover:border-[rgba(0,217,255,0.2)]'
                            }`}
                          >
                        ...
                        <textarea
                          ...
                          className="w-full bg-[#0d1220] border panel-glow rounded-lg p-4 text-white placeholder:text-[#5a6d82] input-glow text-sm leading-relaxed resize-y min-h-[100px] transition-all duration-100"
                          ...
                          <div className="flex items-center gap-3 shrink-0">
                            {results && (
                              <motion.button type="button" onClick={handleExportCSV} whileHover={hoverScale} whileTap={tapScale}
                                className="flex items-center gap-2 px-3 py-2 bg-[#0d1220] hover:bg-[#141c2e] text-[#c8d6e5] rounded-lg text-sm font-medium border panel-glow">
                            ...
                            <motion.button type="submit" disabled={!prompt.trim() || loading} whileHover={prompt.trim() && !loading ? hoverScale : {}} whileTap={prompt.trim() && !loading ? tapScale : {}}
                              className={`flex items-center gap-2 px-5 py-2 rounded-lg text-sm font-bold transition-all duration-100 ${
                                !prompt.trim() || loading ? 'bg-[#141c2e] text-[#5a6d82] cursor-not-allowed' : 'bg-[#00d9ff] hover:bg-[#00f0ff] text-[#0a0e17] hover:shadow-[0_0_25px_rgba(0,217,255,0.4)]'
                  ...
                  <motion.div variants={staggerContainer} initial="initial" animate="animate" className="flex flex-col gap-4">
                    <motion.div variants={staggerItem}>
                      <label
                        className={`flex flex-col items-center justify-center gap-3 py-8 sm:py-10 hud-panel border-dashed cursor-pointer transition-all ${
                          uploadingDoc ? 'border-[rgba(0,217,255,0.5)] bg-[rgba(0,217,255,0.05)]' : 'hover:border-[rgba(0,217,255,0.3)] hover:bg-[#0d1220]'
                        }`}
                      ...
                      >
                        ...
                        {uploadingDoc ? <Loader2 className="w-7 h-7 text-[#00d9ff] animate-spin" /> : <Upload className="w-7 h-7 text-[#5a6d82]" />}
                        <div className="text-center">
                          <p className="text-sm font-medium text-[#c8d6e5]">{uploadingDoc ? 'Uploading…' : 'Drop files here or click to upload'}</p>
                          <p className="text-[11px] text-[#5a6d82] mt-1" style={{ fontFamily: 'JetBrains Mono, monospace' }}>TXT, PDF, MD, CSV, JSON</p>
                        </div>
                      </label>
                    </motion.div>
                    ...
                    {documents.length === 0 ? (
                      <motion.div variants={staggerItem} className="text-center py-16 hud-panel border-dashed">
                        <div className="w-12 h-12 mx-auto mb-4 rounded-xl bg-[#0d1220] flex items-center justify-center">
                          <FileText className="w-6 h-6 text-[#5a6d82]" />
                        </div>
                        <p className="text-[#c8d6e5] text-sm font-medium">No documents uploaded</p>
                        <p className="text-[#5a6d82] text-xs mt-1">Upload files to enable RAG-powered evaluations.</p>
                      </motion.div>
                    ) : (
                      documents.map((doc, idx) => (
                        ...
                        >
                          <div className="flex items-center gap-3 min-w-0">
                            <div className="p-2 bg-[#0d1220] rounded-md border panel-glow shrink-0">
                              <FileText className="w-4 h-4 text-[#5a6d82]" />
                            </div>
                            <div className="min-w-0">
                              <p className="text-sm font-medium text-[#c8d6e5] truncate">{doc.filename}</p>
                              <div className="flex items-center gap-3 mt-0.5">
                                {doc.chunk_count !== undefined && <span className="text-[11px] text-[#5a6d82]" style={{ fontFamily: 'JetBrains Mono, monospace' }}>{doc.chunk_count} chunks</span>}
                                {doc.uploaded_at && <span className="text-[11px] text-[#5a6d82]">{new Date(doc.uploaded_at).toLocaleDateString()}</span>}
                              </div>
                            </div>
                          </div>
                          <motion.button onClick={() => handleDeleteDoc(doc.id)} whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.9 }}
                            className="p-2 text-[#5a6d82] hover:text-[#ff3366] hover:bg-[rgba(255,51,102,0.1)] rounded-md transition-colors shrink-0">
                  ...
                  <motion.div variants={staggerContainer} initial="initial" animate="animate" className="flex flex-col gap-3">
                    {history.length === 0 ? (
                      <motion.div variants={staggerItem} className="text-center py-20 hud-panel border-dashed">
                        <div className="w-12 h-12 mx-auto mb-4 rounded-xl bg-[#0d1220] flex items-center justify-center">
                          <Inbox className="w-6 h-6 text-[#5a6d82]" />
                        </div>
                        <p className="text-[#c8d6e5] text-sm font-medium">No history yet</p>
                        <p className="text-[#5a6d82] text-xs mt-1">Your evaluation results will appear here.</p>
                      </motion.div>
                    ) : (
                      history.map(item => (
                        <motion.div key={item.id} variants={staggerItem} whileHover={cardHover} className="hud-panel p-4 sm:p-5">
                          <span className="text-[11px] text-[#5a6d82]" style={{ fontFamily: 'JetBrains Mono, monospace' }}>{new Date(item.timestamp).toLocaleString()}</span>
                          <div className="text-[14px] text-[#c8d6e5] mt-2 mb-3 bg-[#0a0e17] p-3 rounded-lg border panel-glow leading-relaxed">"{item.prompt}"</div>
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                            {[{ m: item.models.model1, d: item.results.groq_1 }, { m: item.models.model2, d: item.results.groq_2 }].map((col, i) => (
                              <div key={i} className="bg-[#0a0e17] p-3 rounded-lg border panel-glow">
                                <div className="text-[11px] text-[#5a6d82] font-semibold uppercase tracking-wider mb-2 flex items-center justify-between">
                                  <span className="truncate">{col.m}</span>
                                  <span className="text-[#5a6d82] shrink-0 ml-2" style={{ fontFamily: 'JetBrains Mono, monospace' }}>{col.d?.latency_ms || 0}ms</span>
                                </div>
                                <div className="text-xs text-[#c8d6e5] leading-relaxed max-h-28 overflow-y-auto hud-scrollbar whitespace-pre-wrap">{col.d?.content || 'Error'}</div>
                              </div>
                            ))}
                          </div>
                        </motion.div>
                ...
                {activeOverlay === 'settings' && (
                  <motion.div variants={staggerContainer} initial="initial" animate="animate" className="flex flex-col gap-5">
                    <motion.div variants={staggerItem}>
                      ...
                      <p className="text-xs text-[#5a6d82] mb-4">Customize the display names for the AI models.</p>
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                          <label className="text-[11px] font-semibold text-[#5a6d82] uppercase tracking-wider mb-1.5 block" style={{ fontFamily: 'Orbitron, sans-serif' }}>Model 1</label>
                          <input type="text" value={settings.model1Name} onChange={(e) => setSettings({ ...settings, model1Name: e.target.value })}
                            className="w-full bg-[#0d1220] border panel-glow rounded-md px-3 py-2 text-sm text-[#c8d6e5] input-glow transition-all duration-100" style={{ fontFamily: 'JetBrains Mono, monospace' }} />
                        </div>
                        <div>
                          <label className="text-[11px] font-semibold text-[#5a6d82] uppercase tracking-wider mb-1.5 block" style={{ fontFamily: 'Orbitron, sans-serif' }}>Model 2</label>
                          <input type="text" value={settings.model2Name} onChange={(e) => setSettings({ ...settings, model2Name: e.target.value })}
                            className="w-full bg-[#0d1220] border panel-glow rounded-md px-3 py-2 text-sm text-[#c8d6e5] input-glow transition-all duration-100" style={{ fontFamily: 'JetBrains Mono, monospace' }} />
                        </div>
                      </div>
                    </motion.div>
                    <motion.div variants={staggerItem} className="pt-5 mt-4 border-t border-[rgba(0,217,255,0.1)]">
                      <h3 className="text-[15px] font-medium text-white mb-1">Account</h3>
                      <p className="text-xs text-[#5a6d82] mb-3">Signed in as <span className="text-[#00d9ff] font-medium">{user}</span></p>
